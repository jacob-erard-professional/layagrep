# Reprise sur main — 19 septembre 2026
...
Le binaire `dist/cli.js` garde le scaffold : aide/version fonctionnent ; les autres
commandes sortent en 69. `src/cli-commands.ts` est exercé directement par les tests.
Ce choix évite de présenter un produit qualifié avant la clôture des dépendances.

## Ordre de reprise

| Priorité / pilote | Travail | Preuve attendue avant clôture |
| --- | --- | --- |
| S + M : JG-008 | Durcir `source/authorization.ts` et les chemins de configuration | Identité de racine conservée, liens/reparse points sur chaque composant, lectures via handle vérifié, cas Windows/POSIX et courses. Voir `research/source-authorization.md`. |
...
| S + M + J : JG-020/JG-022/JG-025 puis JG-023/JG-026 | Valider les invariants bout en bout, puis raccorder le binaire | Matrice R1–R11, Linux sur le même artefact, parité CLI/MCP et installation complète. |
...
Sur Windows, Node.js 24.15.0 : installation depuis le lockfile, contrôle de types,
336 tests, build et 7 smoke checks réussis lors de la consolidation. Les 14 contrôles
SDK locaux et la validation du corpus, y compris côté curateur, passent aussi. La matrice CI
Windows/Linux existe mais n’a pas tourné ; le nouveau lot n’a pas été vérifié sur Linux.
