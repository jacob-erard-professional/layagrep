...
**État au 19 septembre 2026 :** trois fixtures JS/TS écrites pour le projet (`orders-api`, `subscription-cache`, `migration-audit`, licences et autorisations enregistrées dans chaque manifeste) et 42 questions annotées (30 comportementales, 10 contrôles par identifiant exact, 2 sans preuve, 2 cas ambigus avec preuve alternative) dans `benchmarks/manifests/development/`. Protocole du corpus, empreinte d’arbre, règles de validation et procédure de revue dans `benchmarks/README.md` ; contrôleur sans dépendance `benchmarks/tools/check-corpus.ts` (`npm run corpus:check`) exécuté par la suite hors ligne `tests/corpus.test.ts` (10 cas, dont 8 négatifs qui vérifient que le contrôleur refuse réellement un manifeste faux). Les fixtures délibérément invalides placées sous `tests/fixtures/` sont exclues du contrôle de types par `tsconfig.json` (sinon `npm run typecheck` échoue sur le contenu volontairement cassé) et ne sont jamais exécutées par la suite de tests, dont le glob est volontairement non récursif.

**Reste à faire :** figer le jeu réservé (questions versionnées, réponses hors de l’espace de travail de l’agent) et faire valider le protocole et les annotations par S. **Questions ouvertes pour S :** section « Open questions for the S review » de `benchmarks/README.md`.**Questions ouvertes pour S :** section « Open questions for the S review » de `benchmarks/README.md` (vocabulaire des rôles, granularité des plages, stockage du jeu réservé, fixtures tierces, cas sans preuve, enregistrement des budgets).
