...
**Rendu humain J (disponible) :** `src/cli-render.ts` + `tests/cli-render.test.ts` (7 cas) — en-tête d’état, ligne de couverture, raisons d’arrêt bornées, extraits affichés **verbatim** (sauts de ligne CRLF compris, aucune réindentation ni reformulation), et mesure de la vue humaine elle-même (tokens du compteur de référence et octets) présentée **séparément** du budget de réponse qui appartient à la charge JSON (spécification §4.5). Un résultat partiel indique explicitement que la couverture est incomplète et qu’une sélection vide n’établit pas l’absence ; un refus ou une erreur affiche le code et le conseil de reprise sans inventer de preuve. Le rendu est pur et déterministe, et l’adaptateur stdout ne double pas son saut de ligne final mesuré.

**Branchement J (livré) :** `src/cli.ts` accepte un exécuteur injecté (`CliDependencies.runCommand`) pour les tests et utilise sinon `executeCommand`. Une commande validée est transmise telle quelle (type, config, portée, requête résolue, `--json`) et seuls les codes documentés {0, 2, 3, 4, 130} sont propagés. Un échec sort en 4 avec une ligne de diagnostic : seul un code court en majuscules est repris, jamais le message susceptible de contenir un chemin ou du code.

**Aide J (livrée) :** `src/cli-help.ts` + `tests/cli-help.test.ts` — page d’aide par commande et aide globale. La liste d’options est générée depuis la table que l’analyseur applique (`allowedOptionsFor`), donc l’aide ne peut ni annoncer une option refusée ni en cacher une acceptée. `jevgrep <commande> --help` (et `jevgrep cache clear --help`) sort en 0 avec la page demandée. L’aide distingue le démarrage local du serveur MCP, sans scan ni appel, des appels d’outil qui peuvent demander une évaluation distante sous contrôle de la configuration de confiance.
...
- [x] Les exemples d’aide correspondent à des commandes réellement disponibles ; les différences de rendu ne sont pas dissimulées dans la comptabilité. *(commandes câblées, divulgation MCP explicite et sortie humaine complète mesurée)*
